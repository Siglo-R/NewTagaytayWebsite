...
function BusinessList({ businessList }) {
  ...
  const { setSelectedBusiness } = useBusinessContext(); // Using setSelectedBusiness from the hook
  ...
  return (
    <div>
      <div className='flex overflow-x-auto'>
        <div className='flex overflow-scroll overflow-x-auto scrollbar-hide scroll-smooth' ref={elementRef}>
          {sortedBusinessList.map((item, index) => (
            <div key={index} className='mr-1 mb-6 ' onClick={() => setSelectedBusiness(item)}>
